defmodule PairsOne.Plug.Locale do
  @moduledoc """
  ...
  def call(conn, default) do
    ...
    if is_ietf_tag && locale in PairsOne.Gettext.supported_locales do
  ...
  defp assign_locale!(conn, value) do
    ...
    Gettext.put_locale(PairsOne.Gettext, value)
    conn
    |> assign(:locale, value)
  end

  defp extract_locale(conn) do
    locales = if Blank.present? conn.params["locale"] do
    ...
    locales |> Enum.filter(fn locale -> Enum.member?(PairsOne.Gettext.supported_locales, locale) end)
  end

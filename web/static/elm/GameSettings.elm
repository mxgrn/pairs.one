    ...
    div [ class "new-game-selector" ]
        ...
        , Html.form [ action <| "/" ++ model.locale ++ "/games", method "post" ]
            ...
            , div [ class "btn-go-wrapper" ]
                [ button [ href "", class "btn btn-lg btn-primary btn-go", type' "submit" ] [ text "Go!" ]
                ]
            ]
        ]
...
themeButton model =
    div []
        [ div [ class <| "btn btn-default btn-game-setting " ++ (levelCls model.theme.difficulty), onClick ShowThemeSelector ]
            [ span []
                [ text "Theme: " ]
            , img [ class "theme-icon", src <| "/images/" ++ model.theme.folder ++ "/1.svg" ]
                []
            , span [ class "caret" ]
                []
            ]
...
themeView theme =
    ...
    in
        label [ class <| "btn btn-default btn-theme " ++ (levelCls theme.difficulty), onClick <| SelectTheme theme.name ]
            [ img [ src <| "/images/" ++ theme.folder ++ "/1.svg", width 50, height 50 ] []
